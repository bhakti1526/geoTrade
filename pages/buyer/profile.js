...
const profile = () => {
  ...
  const handleSubmit = async (val) => {
    ...
    if (img !== null || pdf !== null) {
      ...
      await axios
        .post(`${process.env.NEXT_PUBLIC_API_URL}/api/img/img-pdf`, fd)
        .then(async (res) => {
          await axios
            .post(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/user/profile`, {
              ...val,
              img: res?.data?.data?.img || "",
              brocherSheet: res?.data?.data?.pdf || "",
            })
            .then((res) => {
              console.log(res);
            })
