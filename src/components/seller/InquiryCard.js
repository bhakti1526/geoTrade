...
const InquiryCard = ({ data }) => {
  ...
  console.log("data".toUpperCase(), data.createdBy._id);

  const { postData } = usePostAxios("/api/user/chats/gen/chat?inRfq=true");

  const { push, pathname } = useRouter();

  const handleData = async () => {
    await postData({ seller: data.createdBy._id });
    push("/buyer/chats");
  };
        ...
        <div className="row">
          <div className="col-md-12 col-lg-6">
            <div className="lead-left-desc">
              ...
              <h5 className="mb-3">{`${data?.parentGroup?.parentGroupName} - ${data?.parentCategory?.parentCatagoryName} - ${data?.parentSubCategory?.parentSubCategoryName}`}</h5>
                          ...
                          >
                            ...
                            {data?.createdBy?.mobile && (
                              <a
                                ...
                                href={`tel:${data?.createdBy.mobile}`}
                              >
